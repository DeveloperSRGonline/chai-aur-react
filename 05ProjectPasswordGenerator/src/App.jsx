...
const App = () => {
  ...
  // password generator using useCallback (fn,[])
  const passwordGenerator = useCallback(() => {
    // variable to store generated password
    let pass = "";
    // container or bowl or string form which we will take random characters
    let str = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

    // add number also in the original string
    if (number) str += "0123456789";
    // add special characters also in the original string
    if (character) str += "!@#$%^&*()_+-=[]{}|;:,.<>?";

    // generating password
    for (let i = 1; i <= length; i++) {
      // genrating random index to get random char from str
      let char = Math.floor(Math.random() * str.length);
      // string mein se jo bhi random index aaya us position ka char pass mein add karte jao
      pass += str.charAt(char);
    }
    // finally jo last mein jo password aaya vo set kardo password mein
    setPassword(pass);
